	...
	return (
		<Card className="mw-100 mt-0 p-0 text-center border-top-1 border-right-0 border-left-0 border-bottom-0">
			...
			<ListGroup variant="flush">
				<ListGroup.Item className="text-justify" dangerouslySetInnerHTML={ { __html: sprintf( __( 'Goto %1$s and click/drag %2$s and place it where you want it to be.' ), `<b>${__( 'Appearance' )}</b> &gt; <a href="${admin_url}/widgets.php">${__( 'Widgets' )}</a>`, `<b>${__( 'WP Telegram Legacy Widget' )}</b>` ) } }>
				</ListGroup.Item>
