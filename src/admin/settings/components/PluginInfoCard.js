...
export default () => {
	const { wptelegram_widget: { title } } = window;
		...
		<Card border="info" className="mw-100 p-0 text-center">
			...
			<Card.Body>
				<Card.Text className="text-justify">
					{ __( 'Display the Telegram Public Channel or Group Feed in a WordPress widget or anywhere you want using a shortcode.' ) }
				</Card.Text>
			</Card.Body>
			<ListGroup variant="flush">
				<ListGroup.Item>
					<div><span>{ sprintf( __( 'Do you like %s?' ), title ) }</span></div>
					<div><a href="https://wordpress.org/support/plugin/wptelegram-widget/reviews/#new-post" rel="noopener noreferrer" target="_blank" className="text-center text-info ml-1" style={ { textDecoration: 'none' } }><span style={ { color: 'orange', fontSize: '1.5rem' } }>★★★★★</span></a></div>
				</ListGroup.Item>
				<ListGroup.Item>
					<Donate />
				</ListGroup.Item>
