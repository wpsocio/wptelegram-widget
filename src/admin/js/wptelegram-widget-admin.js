(function( $, I18n ) {
    ...
    app1.send_test = function(){
        ...
        if ( 'username' == id  ) {
            if (!app1.username.val().trim()) {
                window.alert(I18n.empty_username);
                return;
    ...
    app1.test_username = function( username, bot_token ){
    	...
    	var new_text = window.prompt(I18n.test_message_prompt,I18n.test_message_text);
    	if (null == new_text)
    		return;
    ...
    app1.handle_username_test = function( jqXHR ) {

        app1.metabox.find('#username-chat-table').removeClass('hidden');

        var col1 = '<td>' + app1.username.val() + '</td>', col2, col3, col4;
        ...
        if ( undefined == jqXHR || '' == jqXHR.responseText ) {
            ...
            col2 = '<td>' + title + '</td>';
            col3 = '<td>' + result.chat.type + '</td>';
            col4 = '<td>'+I18n.success+'</td>';
        }else{
            col2 = '<td>' + JSON.parse( jqXHR.responseText ).description + '</td>';
            col3 = '<td>' + I18n.error + ' ' + jqXHR.status + '</td>';
            col4 = '<td>' + I18n.failure + '</td>';
        }
    ...
    app1.handle_chat_member_count = function ( jqXHR ) {
    	if ( undefined == jqXHR || '' == jqXHR.responseText ) {
            return;
        }
...
})( jQuery, window.wptelegram_widget.I18n );
