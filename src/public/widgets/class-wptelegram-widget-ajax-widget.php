...
class WPTelegram_Widget_Ajax_Widget extends WP_Widget {
    ...
    */
    public function __construct() {
        parent::__construct(
            'wptelegram_widget_ajax_widget', // Base ID
            sprintf( '%1$s %2$s', esc_html__( 'WP Telegram Ajax Widget', 'wptelegram-widget' ), esc_html__( '(Beta)', 'wptelegram-widget' ) ), // Name
            array( 'description' => esc_html__( 'Display the Telegram Public Channel Feed in an ajax widget with infinite scroll.', 'wptelegram-widget' ),
            ) // Args
        );
    }
    ...
    public function form( $instance ) {
        ...
        <p>
            ...
            <input type="text" value="<?php echo esc_attr( $instance['widget_width'] ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'widget_width' ) ); ?>" id="<?php echo esc_attr( $this->get_field_id( 'widget_width' ) ); ?>" class="widefat" placeholder="300, 100%" />
            <br />
        </p>
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'widget_height' ) ); ?>"><?php _e( 'Widget Height', 'wptelegram-widget' ); ?></label>
