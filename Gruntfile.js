					...
					replacements: [
						{
							...
							replacement( match, p1 ) {
								...
								const { config } = grunt;
								const today = new Date();
								const replace = '[' + version + ' - ' + today.getFullYear() + '-' + ( '0' + ( today.getMonth() + 1 ) ).slice( -2 ) + '-' + today.getDate() + '](https://github.com/manzoorwanijk/' + config( [ 'pkg', 'name' ] ) + '/releases/tag/v' + version + ')';
								return match.replace( p1, replace );
							},
						},
					],
